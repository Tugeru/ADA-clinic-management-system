...
export const analyticsApi = {
  ...
  async getConsumptionSummary(period?: string) {
    ...
    return {
      items: items.map((m: any) => ({ id: m.medicineId, name: m.name, totalDispensed: m.totalDispensed })),
      trend: [],
      totalConsumption: items.reduce((s: number, m: any) => s + m.totalDispensed, 0),
      stockAlerts: 0,
      pendingOrders: 0,
    };
  },
...
function mapVisit(v: any): Visit {
  return {
    id: v.id,
    date: v.visitDate?.slice(0, 10) ?? v.timeIn?.slice(0, 10),
    timeIn: v.timeIn?.slice(11, 16),
